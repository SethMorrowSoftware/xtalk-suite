...
primitives, 2-D physics with a game kit, the Nostr protocol, and archive.org —
each behind a
...
for OnionXT, NostrXT and ArchiveXT, pure LiveCodeScript), built to one shared
set of engineering rules so the nine read as one system. They interoperate: your
...
| **[archivext](archivext/)** | `ax*` | archive.org's public APIs (pure script over the engine's Internet library) | Added 2026-09-15: the Internet Archive from xTalk - advanced search with a Lucene grammar and a user-text sanitizer, item metadata, download / streaming / thumbnail / details / embed URLs, the preset catalogues of three shipped archive.org web apps (Live Music Archive bands, LibriVox categories, Moving Image Archive collections) plus a generic any-mediatype family, an owned JSON reader, and a playlist engine that turns an item's raw file list into a concert's set list, a book's chapters or a film's episodes with quality variants; an asynchronous fetch layer with one callback per request |
...
| archivext | no — pure LiveCodeScript (over the engine's Internet library) | n/a | New 2026-09-15, and **first engine contact the same day**: `axSelfTest` ran on the user's OXT engine at **357 passed / 2 failed / 0 skipped**; both reds fixed that day (the JSON reader's by-key index met the engine's array-key case folding, `docs/OXT-ENGINE-NOTES.md` 2.7; and one wrong hand-written expectation), the re-run read 363/0/0, the demo booted with its self-check 11/11, and **the first live pass went green the same evening**: three legs from the demo's Live probe (a LibriVox search, 200, numFound 101; a real item's metadata, 200, 196 KB chunked; the movies family's cheap scope, 200, 17 million hits), after the film club's full video scope had drawn a 400 and a timeout. The next day (2026-09-16) the five-leg probe and the demo's Search button closed four more: a live 412-file list through the parser and playlist engine (58 chapters), the async `search` kind through the callback (four searches, three families), and both documented error shapes refused by the shipped code. Still owed: the suite-paste fold, the async `item` kind, the scrape API, streaming, TLS's certificate direction. What IS machine-verified headlessly, on every build: the whole `ax*` pure layer is EXECUTED through the family interpreter against `tools/archive_reference.py` (an independent implementation written from the three source apps' own JavaScript and PHP, anchored to their published test vectors) over nine synthetic fixtures - 2920 checks on 2026-09-15 - and `tools/test-script-vectors.py` proves that gate fails on seven seeded defects. The fixtures are SYNTHETIC (archive.org is unreachable from the build sandbox), so a green build says the script agrees with the oracle about those bytes, not that the live site still answers in that shape - the live pass said the search shape does, and the metadata body arrived whole; the fetch layer (`load URL ... with message`, `put URL`) is the suite's first libURL engine record (`docs/OXT-ENGINE-NOTES.md` 6.9: https works, chunked bodies arrive whole, `libURLLastRHHeaders` keeps the LAST reply received), and whether it VERIFIES certificates is still the open TLS question. The 12-section `axSelfTest` is folded into the suite paste as `ax1*` and the three-apps-in-one demo carries the library and the harness verbatim. A SECOND demo landed 2026-09-20 and met an engine the same day: `archivext/examples/archive-gallery.livecodescript` drives the same library as a PICTURE gallery - twenty-four thumbnails per search in engine IMAGE objects, a film or a concert on a dark stage in the engine's own PLAYER object, a photograph in an image object - which makes it the first stack in this suite to put fetched bytes into an image object from a script. That run gave section 5 of `docs/OXT-ENGINE-NOTES.md` its first two entries about media: **5.7**, an image object takes a fetched JPEG (10,066 bytes read back as 180x124, 1.1 MB as 1988x1367) and a REFUSAL keeps the rect the control already had rather than answering zero - which is why box2dxt's width-under-two idiom needs a control with no rect; and **5.8**, a player can open an https MP3 stream, report a duration, fire `playStarted`, advance its clock six seconds and be SILENT, so no property of a player object distinguishes playing from running-and-silent. The stack now sets `the playLoudness` on the player and the engine (it never had), logs `the tracks` and `the mediaTypes`, gives the reader a volume control, and retries once over `http://` - the DirectShow-versus-https experiment nobody in this suite has run. It caches on two tiers (thumbnails and metadata in memory and on disk, search pages in memory only) and queues its fetches, because archive.org is a public service run on donations |
...
broadly open: the
**macOS engine pass** (every member ships a universal-mac dylib since release
run 12, 2026-08-27 - box2dxt had all five platforms from its 2026-08-14 fold,
the other five landed or refreshed theirs in that run - but no OXT engine has
loaded any of them yet), and the
...
put axVersion()          -- archivext, e.g. "ArchiveXT 0.1.0" (start using stack "archivext" first)
```

Or run all nine suite members plus both app layers - Riptide and holde-em - at once.
...
2026-08-23, nothing in it engine-observed yet), archivext's 12-section
`axSelfTest` (the query grammar and sanitizer, the owned JSON reader, the
response parsers and the playlist engine over hex-pinned synthetic fixtures -
folded in 2026-09-15, not yet engine-observed), box2dxt's `stSelfTest` (51 handlers, 377 `stAssert` call sites of which 375 run, at
...
**963 of 977 public handlers** (re-measured 2026-09-15, when archivext folded in
at 99/99 with zero exemptions; the gate prints today's number):

| sodiumxt | onionxt | coinxt | torrentxt | enetxt | datachannelxt | riptide | nostrxt | box2dxt (Kit) | archivext |
|---|---|---|---|---|---|---|---|---|---|
| 73/73 | 31/45 | 90/90 | 85/85 | 23/23 | 31/31 | 83/83 | 101/101 | 313/313 | 99/99 |
...
- **`release-binaries.yml`** — the assembly step, run by hand
  ...
  the decision is pressing "Run workflow". Since 2026-08-23 it carries **macOS lanes for all six members** (single-pass universal builds for sodiumxt, coinxt, enetxt, box2dxt; a two-slice-lipo job for torrentxt and datachannelxt, whose OpenSSL-bearing stacks cannot single-pass), building genuinely UNIVERSAL dylibs - both slices cross-compiled in one pass, `lipo -archs` asserted at birth, arm64 tested natively and x86_64 under Rosetta 2, unsigned in the distribution sense (the linker's ad-hoc signature; no notarization anywhere). **The first dispatch ran 2026-08-26 and all six mac lanes went green** on their first execution, but landed nothing: one unrelated lane failed (`torrentxt x86_64-linux`, a missing Perl module) and the commit stage gates on the whole 30-job matrix. Runs 10 and 11 each cleared one more stage and found one more real defect (enetxt's unfiltered mac export table; the commit allowlist that could not spell box2dxt). **Run 12 went end to end on 2026-08-27** - the first dispatch to reach its commit stage, landing every member's universal-mac dylib and rebuilding all the Linux/Windows binaries. The trap that kept macOS out until then: `macos-15` runners are arm64-only, so they
