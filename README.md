...
| archivext | no — pure LiveCodeScript (over the engine's Internet library) | n/a | New 2026-09-15, and **first engine contact the same day**: `axSelfTest` ran on the user's OXT engine at **357 passed / 2 failed / 0 skipped**; both reds fixed that day (the JSON reader's by-key index met the engine's array-key case folding, `docs/OXT-ENGINE-NOTES.md` 2.7; and one wrong hand-written expectation), with the re-run, the suite-paste fold, the demo's window and the live archive.org legs still owed. What IS machine-verified headlessly, on every build: the whole `ax*` pure layer is EXECUTED through the family interpreter against `tools/archive_reference.py` (an independent implementation written from the three source apps' own JavaScript and PHP, anchored to their published test vectors) over nine synthetic fixtures - 2920 checks on 2026-09-15 - and `tools/test-script-vectors.py` proves that gate fails on seven seeded defects. The fixtures are SYNTHETIC (archive.org is unreachable from the build sandbox), so a green build says the script agrees with the oracle about those bytes, not that the live site still answers in that shape; the fetch layer (`load URL ... with message`) has no engine record, and libURL's https behaviour is the suite's open TLS question. The 12-section `axSelfTest` is folded into the suite paste as `ax1*` and the three-apps-in-one demo carries the library and the harness verbatim |
