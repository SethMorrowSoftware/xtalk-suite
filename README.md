...
|---|---|---|---|
| sodiumxt | 5 platforms, ABI 10 | full `sxSelfTest()` 106/106 incl. ChaCha20 in the suite paste on Linux, 2026-09-25 (ChaCha20's first recorded Linux run; the bitness and build not recorded), and on Windows, 2026-09-24, on a shipped MSVC DLL by the maintainer's account (the 2026-09-12 build's first engine load; the bitness not recorded) | the MSVC DLL of the other bitness; a Linux run that names its bitness; a first Mac load; the demo re-pass |
| torrentxt | 5 platforms, ABI 11 | harness 106/106 in the suite paste on Linux, 2026-09-25 (the 996-byte BEP44 caps included, so a build from 2026-09-08 source or later, INFERRED; the bitness and build not recorded), and on Windows, 2026-09-24: the 2026-09-12 build's first engine load by the maintainer's account, and the first recorded libtorrent 2.1 run on Windows; rp1 chat reported working across two machines on one LAN, 2026-08-27 | a Linux run that names its row and build; demo re-opens; the Tor paths; two-machine seed/leech and resume; a real public swarm |
| enetxt | 5 platforms, ABI 2 | folded 34 checks green and the paste's live loopback COMPLETED in the suite paste on Linux, 2026-09-25 (the build not recorded); folded 34 on the 2026-09-12 DLL, Windows, 2026-09-24 (the paste's loopback stalled there, runbook 5.5); standalone async loopback, 2026-08-13 | an async loopback on the 2026-09-12 builds; LAN chat between two machines; internet chat across two networks |
| datachannelxt | 5 platforms, ABI 1 | folded 39 checks green and the paste's live loopback COMPLETED in the suite paste on Linux, 2026-09-25 (the build not recorded); folded 39 on the 2026-09-12 DLL, Windows, 2026-09-24 (the paste's loopback stalled there, runbook 5.5); standalone async loopback, 2026-08-15; a DHT-signalled chat reported working across two machines on one LAN, 2026-08-27 | an async loopback on the 2026-09-12 builds; browser interop; a call across two networks; the loopback demo |
...
| nostrxt | pure script | core 277/0/2 in the suite paste, Linux 2026-09-25 and Windows 2026-09-24 (the NIP-44 cipher round trip included); relay connect, publish and ok-confirm live against wss://nos.lol, 2026-08-24 | the relay receive leg; NIP-42; `ws://`; bad-certificate TLS (engine note 6.8) |
| box2dxt | 5 platforms, ABI 4 | Kit harness v32: 385/0 in the suite paste on its 1200-wide card, Linux 2026-09-25 and Windows 2026-09-24 (on the 2026-09-12 DLL there); both runs settle the itemDelimiter half of engine note 2.3 (handler-local). `playLoudness` reads back 0 on Linux (engine note 5.4), a printed note since v31 | the five game stacks since the 2026-08-14 fold; risk R1 on Linux and macOS |
| coinxt | 5 platforms, ABI 7 | library 296/296 at ABI 7 in the suite paste, Linux 2026-09-25 and Windows 2026-09-24 (on the 2026-09-12 DLL there; the six `cxPubkeyCombine` checks included); 290/290 at ABI 6, 2026-08-24; the wallet's engine logs, 2026-09-01 to 09-03: all four public transports, testnet broadcasts, RBF, CPFP, an inscription | silent-payment receive; the wallet surface added since 2026-09-04; a native-P2WPKH and any Ethereum broadcast; a run that names its Windows DLL's bitness |
| riptide | app | phases 1-4 done on two machines, 2026-08-13 and 2026-08-15; harness 489/0/2 in the suite paste, Linux 2026-09-25 (its third probe line read the engine's comparison constant, 10 DBL_EPSILON, engine note 2.10) and Windows 2026-09-24 (the day's second and third runs; its first read 487/2/2: a stale capability count, and a u64 bound the engine let through; both fixed and green that evening) | live passes of phases 5-8; the faststart re-run; the boot re-paste |
...
suite paste, by the maintainer's account (the bitness was not recorded). A Linux engine ran the same
paste green on 2026-09-25 with all six loaded, but the record says neither which bitness nor
whether the committed `.so` files were installed, so no Linux row has a record of its own yet. No
