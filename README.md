...
|---|---|---|---|
| sodiumxt | 5 platforms, ABI 10 | full `sxSelfTest()` 106/106 incl. ChaCha20 in the suite paste on Linux, 2026-09-25, on the `x86_64-linux` file (run 12's) by the maintainer's account (64-bit; ChaCha20's first recorded Linux run), and on Windows, 2026-09-24 and 09-25, on the `x86_64-win32` MSVC DLL by the maintainer's account (64-bit; the 2026-09-12 build's first engine load) | the 32-bit MSVC DLL, and `sxVersion()` on Windows; the 32-bit Linux library; a first Mac load; the demo re-pass |
| torrentxt | 5 platforms, ABI 11 | harness 106/106 in the suite paste on Linux, 2026-09-25, the 2026-09-12 `x86_64-linux` build's first engine load by the maintainer's account (64-bit; its 996-byte BEP44 refusals agree), and on Windows, 2026-09-24 and 09-25, on the `x86_64-win32` build by the maintainer's account (64-bit; its first engine load, and the first recorded libtorrent 2.1 run on Windows); rp1 chat reported working across two machines on one LAN, 2026-08-27 | the 32-bit Linux and Windows libraries; demo re-opens; the Tor paths; two-machine seed/leech and resume; a real public swarm |
| enetxt | 5 platforms, ABI 2 | folded 34 checks green and the paste's live loopback COMPLETED in the suite paste on Linux, 2026-09-25, on the 2026-09-12 `x86_64-linux` build by the maintainer's account; folded 34 on the 2026-09-12 `x86_64-win32` DLL, Windows, 2026-09-24 and 09-25 (the paste's loopback stalled there all four times, runbook 5.5); standalone async loopback, 2026-08-13 | the standalone selftest's own async loopback on the 2026-09-12 builds (the paste's own completed on Linux); LAN chat between two machines; internet chat across two networks |
| datachannelxt | 5 platforms, ABI 1 | folded 39 checks green and the paste's live loopback COMPLETED in the suite paste on Linux, 2026-09-25, on the 2026-09-12 `x86_64-linux` build by the maintainer's account; folded 39 on the 2026-09-12 `x86_64-win32` DLL, Windows, 2026-09-24 and 09-25 (the paste's loopback stalled there all four times, runbook 5.5); standalone async loopback, 2026-08-15; a DHT-signalled chat reported working across two machines on one LAN, 2026-08-27 | the standalone selftest's own async loopback on the 2026-09-12 builds (the paste's own completed on Linux); browser interop; a call across two networks; the loopback demo |
