...
| box2dxt | yes | **all 5 platforms** + `MANIFEST.sha256` (added in the fold) | The family ancestor, mature and feature-frozen upstream. The 2026-08-14 fold replaced its pre-unification checker with the suite's (first contact found ~1550 violations: mostly the pre-ASCII character set, plus 29 real `repeat with ... step` engine traps in the platformer, all fixed). **The folded `stSelfTest` ran on a real engine THREE times 2026-08-16** (346/11, then 348/9, then 353/4 at harness v25): the engine corrected eight blind first-contact coverage tests with its own numbers and found two real Kit defects nothing headless could reach - sprite entry points throwing on an empty control ref (guarded per the family no-op law, finished across the whole sprite surface at v26) and all eight event-buffer readers answering STALE controls after `b2kEventsReset` (count-guarded). Three pre-existing behaviour failures (ghost-layer filtering, post-`b2kClear` gravity, the crawl stall) are open for engine investigation; the games and the raw `b2*` layer still owe their own re-pass |
