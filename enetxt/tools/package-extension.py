...
WHAT IT DOES
    Copies a freshly built `enetxt.{so,dll,dylib}` into the committed
    extension tree at:

        src/code/<arch>-<platform>/enetxt.{so,dll,dylib}

    Installing the packaged extension makes the OXT engine resolve the
    `c:enetxt>` binding automatically via `the revLibraryMapping` - no loose
...
THE BARE-TOKEN NAME RULE
    The file is named with the bare token `enetxt` (NO `lib` prefix) so it
    matches the binding string. A build that emitted `libenetxt.so` is wrong;
...
USAGE
    ...
    python3 tools/package-extension.py --platform-id universal-mac --lib out/enetxt.dylib
...
# (The committed file is always the bare token `enetxt` + this suffix.)
PLATFORM_SUFFIX = {
    "x86_64-linux": ".so",
    "x86-linux": ".so",
    "x86_64-win32": ".dll",
...
def _candidate_basenames(suffix):
    return ["enetxt" + suffix, "libenetxt" + suffix]
...
def strip_in_place(path):
    """Strip a committed ELF/PE artifact of everything the engine does not need.

    A committed binary is a SHIPPED artifact: its debug symbol table and the
    absolute build paths baked into it are pure payload for every user, and the
    paths leak the build machine's directory layout into the release. `strip
    --strip-unneeded` removes the static symbol table and debug sections while
    keeping .dynsym intact, which is the only table the engine's `c:enetxt>`
    bindings resolve through - so the extension keeps working and gets smaller.
...
def copy_into_tree(src_lib, platform_id, dry_run):
    """Copy `src_lib` to src/code/<platform_id>/enetxt<suffix>, creating the
    ...
    dest = os.path.join(dest_dir, "enetxt" + suffix)  # ALWAYS the bare token
...
def assemble_staging(dry_run):
    ...
    if os.path.isdir(CODE_ROOT):
        for pid in VALID_PLATFORM_IDS:
            ...
            lib = os.path.join(CODE_ROOT, pid, "enetxt" + suffix)
            if os.path.exists(lib):
                actions.append((lib, os.path.join(staging, "code", pid, "enetxt" + suffix)))
...
def main(argv):
    ...
    suffix = PLATFORM_SUFFIX[platform_id]
    print("enetxt package-extension: target platform-id = %s (enetxt%s)"
          % (platform_id, suffix))
    ...
    if args.lib:
        ...
        if src_lib is None:
            print("ERROR: no enetxt%s (or libenetxt%s) found under %s.\n"
    ...
    if os.path.basename(src_lib).startswith("lib"):
        print("  note: source is lib-prefixed; writing the BARE-TOKEN name (enetxt%s)." % suffix)
