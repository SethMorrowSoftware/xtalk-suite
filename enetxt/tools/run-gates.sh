#!/usr/bin/env bash
# run-gates.sh - enetxt's OWN gate list: what CI and a contributor run.
# Compiler-free: python3 and sha256sum are all it needs (the native smoke and
# handle tests are the CMake build's, not this list's).
#
...
# have its mechanically named `constant k...` in the .lcb at the same value -
# a missing constant, a wrong value and a value swap all fail here rather
# than on an engine.
echo "== enetxt: tools/check-record-registry.py =="
...
echo "== enetxt: src/code/MANIFEST.sha256 =="
