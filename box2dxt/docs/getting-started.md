...
> and pinned by `MANIFEST.sha256`, so this is already done in the repo. Prefer
> building the library yourself? See [building.md](building.md).
...
scenes at runtime, and bundles a copy of the Kit, so it runs from a single paste.
...
| `b2Version()` returns a different number | Your `box2dxt.lcb` and native library are from different versions. Package and install both from one checkout (the `.lcb` and its `src/code/` libraries travel together), or rebuild the library from the same source. |
| Library won't load on an older PC (Linux/Windows) | The committed libraries are already built without AVX2, so this mostly bites a library you built yourself: rebuild with `-DBOX2D_AVX2=OFF` or `-DBOX2D_DISABLE_SIMD=ON` (see [building.md](building.md#platform--cpu-notes)). On older 32-bit Linux, note the committed `x86-linux` library needs glibc 2.34 (see building.md). |
