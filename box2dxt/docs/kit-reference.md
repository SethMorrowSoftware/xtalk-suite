...
Spritesheet animation on the **icon-button backend** (chosen by the Game Kit's
2026-06-10 OXT spike benchmarks): a sheet registers named or numbered frame *regions* of one
...
Performance notes (measured 2026-06-10 on modest Win32 hardware): a frame
...
**Built-in actions**: **duck** (DOWN while grounded brakes to a crouch - the
hitbox does *not* shrink unless `duckScale < 1`, below), **drop-through**
...
**Swim.** While the player's centre is inside a
...
**Opt-in actions**, each enabled through a knob (the defaults leave the
...
climb/swim), **duck capsule reshape** (`duckScale < 1` turns the brake-duck
...
| `b2kPlayerState()` | `idle` / `run` / `jump` / `fall` / `duck` / `climb` / `hurt` / `swim` / `wallslide` / `dash`, plus `land` for exactly one frame on touch-down (dust puffs, sounds - read it in `on b2kFrame`). A drop-through renders as `fall`; a knockback's own landing shows no `land` tick. The opt-in states (`wallslide`, `dash`) appear only when their knobs are enabled. |
...
**Opt-in action keys** - all off by default (these defaults leave the controller
...
A clipped **viewport group** the loop scrolls (the mechanism the 2026-06-10
...
> **One-way chains:** every `b2kChain`/`b2kSmoothGround` chain
