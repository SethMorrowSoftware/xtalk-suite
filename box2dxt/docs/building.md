...
> **Build note.** Box2D v3.1.0 builds its own sources with `-Werror` *and* the
> CMake `COMPILE_WARNING_AS_ERROR` property, so on newer compilers (e.g. GCC 13+)
> a harmless upstream `-Wmaybe-uninitialized` would fail the build. Our
> `CMakeLists.txt` neutralises both: Box2D's warnings stay visible but non-fatal.
...
You don't deploy this file by hand. Box2Dxt ships as an **extension with the
native library bundled inside it**, under `src/code/<arch>-<platform>/`. Those
libraries are **committed** (see [Continuous integration](#continuous-integration)),
so a fresh clone is already a ready-to-build extension.
`tools/package-extension.py` refreshes that tree from a newer build - point each
flag at the matching library:
...
`lib` prefix) to match the `c:box2dxt>` binding name. Open `src/box2dxt.lcb` in
OXT's **Extension Builder** and **Package** it into `box2dxt.lce` (the `code/`
libraries roll in), then install it via the Extension Manager - or **Test** to
compile and load in place. The engine then loads the right library for the
running platform (Windows, macOS, Linux; LiveCode Community 9.6.3 and OpenXTalk)
with **no download, renaming, sudo, `/usr/lib` or `LD_LIBRARY_PATH`**. Confirm
with `put b2Version()` → `4`.
...
The supported install is the packaged extension: **Package** `src/box2dxt.lcb`
into `box2dxt.lce` and ship that (or the source tree with its `code/` folder).

To hand someone a ready-to-run game as one self-contained zip (no repo, no
toolchain, no internet), `tools/make-release.py` bundles the extension (the
`.lcb` plus its `code/<arch>-<platform>/` libraries), the C shim + Kit for
reference, the demo's spritesheets, a built **and saved** stack, and the
...
- **AVX2 / SIMD.** Box2D assumes **AVX2** on x64 by default. If your binary must
  run on older CPUs, configure Box2D with `-DBOX2D_AVX2=OFF` (keeps SSE2) or
  `-DBOX2D_DISABLE_SIMD=ON` (scalar, slower). The release rows build 64-bit
  with `-DBOX2D_AVX2=OFF` and 32-bit with `-DBOX2D_DISABLE_SIMD=ON`. The
  committed `universal-mac` dylib predates those rows; its x86_64 slice has no
  AVX (VEX/ymm) instructions (disassembled 2026-09-24).
...
The suite's
[`native-box2dxt.yml`](https://github.com/SethMorrowSoftware/xtalk-suite/blob/main/.github/workflows/native-box2dxt.yml)
builds and tests a 5-target matrix (Linux x86-64 and i686 in manylinux2014,
glibc 2.17; macOS universal; Windows x64 and x86) plus an ASan/UBSan smoke-test
lane on every push touching this member, uploading **artifacts** only. The
committed Linux and Windows libraries come from the suite's manual
