...
9. **u64 bound.** `rsReadBEu64` returns empty past 2^53 and ALL THIRTEEN call sites refuse the
   ...
   probe line read 10 to the digit on Linux on 2026-09-25 (`N is N + 1` true at N =
   450359962737050, false one below). So integers 1
   ...
   reads its consequences (wide integers, near zero, number-like text), and read them on Linux
   (2026-09-25) exactly as the source's rule predicts. Tier 1c holds the rule to every recorded
   numeric answer of the three (probe 3's first two items are a text parse the interpreter does
   not model, so they stay out).
...
| 2026-09-25 | OXT on Linux: 64-bit, Kubuntu 24.04, the committed x86_64-linux libraries (the maintainer's account, "latest builds"; the OXT build not recorded) | the D-23 suite paste as at `cba3130`, last regenerated at `f1346e0` (the report's holde-em v0.25.5 / harness 47 header and this harness's third probe line exist together only from there); riptide's folded harness as in the third run plus the third probe line (a diagnostic, not a check; the library changed in comments only) | 489 passed, 0 failed, 2 skipped (the live-tor legs), row 491 / 0 / 2; the whole paste 2672 / 0 / 10, no failures. The first record naming Linux with this member's count. All ten probe keys true bar `hasNostrRelay` (not in the paste by design). Probes 1 and 2 read as on Windows, `true,false,false,false` and `true,false,16,16`, so the same relative rule holds on this build. Probe 3, its first reading anywhere, read `true,true,false,true,false,true,true,false`, the engine source's prediction item for item (OBSERVED, Linux): `"1e999" is "2e999"` and `"1e5" is "100000"` true (text becomes a number: two overflowing texts are equal, exponent form is a number; suite engine note 2.11); `1e-15 > 0` false and `1e-14 > 0` true (the absolute branch near zero); `2^49 < 2^49 + 1` false and `2^48 < 2^48 + 1` true; `N is N + 1` true at N = 450359962737050 and false one below, which puts the constant at 10 DBL_EPSILON to the digit (2^52 / 10 = 450359962737049.6 lies between; suite engine note 2.10). INFERRED to be a fresh launch's Run all (the stack was "Untitled 1" and the boot check saw the run start on open; the maintainer did not say whether another followed), so it does not count as the second Run all in one launch the paste's stale-session fix is owed |
...
- Harness sections added after 2026-08-24 (Nostr, app state, the watermarks, the u64 bound, the 996
  ...
  Linux (the ledger); it has no Windows reading.
