...
wherever XTALK_SIBLING_RIPTIDE / XTALK_SIBLINGS point. The runner and its
oracle are both needed before anything loads, so an absent riptide stops
this gate first, with the clone to run.
...
ORACLE = os.path.join(sibling("riptide"), "tools", "riptide_reference.py")
# Both are needed before anything else loads, so their absence is settled
...
for _path in (RUNNER, ORACLE):
    if not os.path.isfile(_path):
        print("check-script-vectors: %s is not present: it belongs to the "
              "riptide member, which is not beside this checkout. Clone "
              "https://github.com/SethMorrowSoftware/RipTide beside this "
              "checkout as ../riptide, or point XTALK_SIBLING_RIPTIDE / "
              "XTALK_SIBLINGS at it." % _path, file=sys.stderr)
        sys.exit(2)
