...
reader who copies it out gets `handler not found`, and every other gate
stays green about it). ArchiveXT ships the same two mechanical properties
from day one:

  1. DOCUMENTED -> SHIPPED. Every `ax*` token in the scanned docs is a
     public handler, a private handler (CLAUDE.md and the numbered docs are
     internals records and legitimately name internals - a rename still
     breaks the reference and is still caught), a harness handler, or
     listed in NOT_A_HANDLER below with a category and a written reason.
  2. SHIPPED -> DOCUMENTED. Every public handler of the source file is
     named in docs/05-api-reference.md. Complete with zero exemptions when
     the gate landed, so it is a ratchet, not a backlog.
...
USAGE
    python3 archivext/tools/check-doc-handlers.py [--check]
...
SOURCES = [
    os.path.join(MEMBER, "src", "archivext.livecodescript"),
]
# The harness and the demo define real names the docs may cite (axSelfTest
# above all), but their surfaces are documented in README/the plan, not the
# api-reference, so they feed the KNOWN set only - never the must-be-
# documented set.
EXTRA_KNOWN_SOURCES = [
    os.path.join(MEMBER, "examples", "archivext-tests.livecodescript"),
    os.path.join(MEMBER, "examples", "archivext-demo.livecodescript"),
]
API_REFERENCE = os.path.join(MEMBER, "docs", "05-api-reference.md")
DOC_FILES = [
...
TOKEN_RE = re.compile(r"\b(ax[A-Z]\w*)\b")
...
def main(argv):
    ...
    for path in DOC_FILES:
        ...
        for name in set(TOKEN_RE.findall(text)):
            ...
            if name not in known and name not in NOT_A_HANDLER:
                problems.append(
                    f"{rel}: `{name}` is not a handler in the source "
    ...
    for name in sorted(public):
        if not re.search(r"\b" + re.escape(name) + r"\b", api_text):
            problems.append(
                f"docs/05-api-reference.md does not name the public handler "
