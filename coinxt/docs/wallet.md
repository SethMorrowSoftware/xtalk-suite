...
**The library has seven engine passes** (2026-08-08 through 2026-09-25, the
last two at 296/296 in the suite paste, on Windows and then Linux), so the
cryptography under the wallet is engine-observed, `cxPubkeyCombine` (ABI 7,
which the silent-payment receiver uses) included since 2026-09-24. The
receiver built on it has not run.
