...
- **Engine passes of the library:** 2026-08-08 (phase 1, the hash surface); 2026-08-10 (phases 2-4,
  ...
  paste on Windows); 2026-09-25 (296/296 again, the suite paste on Linux x86_64). Every handler (95 of
  95) has now run green on an engine.
- **ABI 7, `cxPubkeyCombine` (2026-09-10):** ran green on an engine on 2026-09-24 (the suite paste on
  ...
  64-bit Kubuntu 24.04, the latest committed builds), its first engine load. The Windows run's bitness
  was not recorded, so the `x86-win32` DLL may still never have executed anywhere, and no engine has
  loaded the `x86-linux` or mac builds.
