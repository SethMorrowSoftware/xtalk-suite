 ...
 * do NOT need libdatachannel and carry the binding's nastiest bug surface:
 *
 *   - dcx_record.h  : the big-endian, length-prefixed KV framing + the
 *                     measure-or-write / -needed overflow contract.
 *   - dcx_handle_table.h : generation-tagged handles -> stale = no-op, never a
 *                     crash, never a recycled-slot alias.
 *
 * Compiles and runs ANYWHERE (no libdatachannel, no OpenSSL), so it is the
...
#include "../src/dcx_handle_table.h"
...
static void test_record_roundtrip() {
    ...
    {
        KVRecord r(w);
        r.put_int(F_DC_PEER, 65537);
        r.put_int(F_DC_CHANNEL, 65538);
        r.put_str(F_DC_LABEL, "chat");
        r.put_str(F_DC_SDP_TYPE, "offer");
        r.put_int(F_DC_STATE, 2);
        unsigned char payload[3] = {0xFF, 0x80, 0x00};
        r.put_bytes(F_DC_PAYLOAD, payload, sizeof payload);
        r.put_str(F_DC_ERROR, "");  /* empty value field is legal */
        r.finish();
    }
    CHECK(!w.overflow());
    ...
    CHECK(fields[0].id == F_DC_PEER && fields[0].type == FT_INT && fields[0].as_int() == 65537);
    CHECK(fields[1].id == F_DC_CHANNEL && fields[1].as_int() == 65538);
    CHECK(fields[2].id == F_DC_LABEL && fields[2].type == FT_UTF8);
    CHECK(fields[2].text() == "chat");
    CHECK(fields[3].id == F_DC_SDP_TYPE && fields[3].text() == "offer");
    CHECK(fields[4].id == F_DC_STATE && fields[4].as_int() == 2);
    CHECK(fields[5].id == F_DC_PAYLOAD && fields[5].type == FT_RAW && fields[5].len == 3);
    CHECK(fields[5].val[0] == 0xFF && fields[5].val[1] == 0x80 && fields[5].val[2] == 0x00);
    CHECK(fields[6].id == F_DC_ERROR && fields[6].len == 0);
}
...
static void test_record_overflow() {
    ...
    {
        KVRecord r(probe);
        r.put_str(F_DC_LABEL, "hello");
        r.put_int(F_DC_PEER, 42);
        r.finish();
    }
 ...
 * [kvrecord]. Mirrors exactly what the shim emits in dcx_poll; the Python
...
static void write_event_entry(RecordWriter &w, uint16_t eventType,
    ...
    size_t bodyStart = w.pos();
    {
        ...
        if (text) r.put_str(F_DC_TEXT, text);
        r.finish();
    }
...
static void test_drain_framing() {
    ...
    write_event_entry(w, E_MESSAGE, {{F_DC_PEER, 7}, {F_DC_CHANNEL, 3}}, "hi");
    write_event_entry(w, E_CHANNEL_OPEN, {{F_DC_PEER, 7}, {F_DC_CHANNEL, 3}});
    CHECK(!w.overflow());
    ...
    CHECK(f1[2].id == F_DC_TEXT && f1[2].text() == "hi");

    /* entry 2 */
    uint16_t t2 = RecordReader::rd_u16(rr.cursor());
    rr.skip(2);
    rr.skip(2);  /* bodyLen */
    CHECK(t2 == E_CHANNEL_OPEN);
    std::vector<Field> f2;
...
static void test_reader_truncation() {
    ...
    {
        KVRecord r(w);
        r.put_str(F_DC_LABEL, "truncate-me");
        r.finish();
    }
...
static void test_golden_vectors() {
    /* B: { F_DC_LABEL(8) utf8 "ab" } */
    {
        uint8_t buf[64];
        RecordWriter w(buf, sizeof buf);
        KVRecord r(w);
        r.put_str(F_DC_LABEL, "ab");
        r.finish();
        const uint8_t exp[] = {0x00, 0x01, 0x08, 0x02, 0x00, 0x02, 0x61, 0x62};
        expect_bytes("record-B", buf, w.pos(), exp, sizeof exp);
    }
    /* A: { F_DC_DROPPED(13) int 42, F_DC_STATE(7) real 0.5 } — the real is a
     * pure FRAMING pin (no dcx field is real-typed today; the codec still is). */
    {
        uint8_t buf[64];
        RecordWriter w(buf, sizeof buf);
        KVRecord r(w);
        r.put_int(F_DC_DROPPED, 42);
        r.put_real(F_DC_STATE, 0.5);
        r.finish();
        const uint8_t exp[] = {0x00, 0x02, 0x0D, 0x00, 0x00, 0x02, 0x34, 0x32,
                               0x07, 0x01, 0x00, 0x03, 0x30, 0x2E, 0x35};
        expect_bytes("record-A", buf, w.pos(), exp, sizeof exp);
    }
    /* Drain: 1 event, E_MESSAGE(8), { F_DC_PEER(1)=7, F_DC_CHANNEL(2)=3,
     * F_DC_TEXT(11) "hi" } */
    {
        uint8_t buf[64];
        RecordWriter w(buf, sizeof buf);
        ...
        const uint8_t exp[] = {0x00, 0x01,                          /* eventCount */
                               0x00, 0x08, 0x00, 0x12,              /* type, bodyLen=18 */
                               0x00, 0x03,                          /* kv count */
                               0x01, 0x00, 0x00, 0x01, 0x37,        /* id1 int "7" */
                               0x02, 0x00, 0x00, 0x01, 0x33,        /* id2 int "3" */
                               0x0B, 0x02, 0x00, 0x02, 0x68, 0x69}; /* id11 utf8 "hi" */
