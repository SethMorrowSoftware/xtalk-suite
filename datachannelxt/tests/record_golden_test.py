#!/usr/bin/env python3
"""record_golden_test.py - byte-exact reference for the DataChannelXT KV record
wire format. A PURE reference: no C++ build, no libdatachannel, runs anywhere.
It re-derives the framing independently of src/dcx_record.h and pins known byte
...
# ------------------------------------------------------------------ registries
# Mirror of src/dcx_record.h (kept tiny - only what the vectors touch). The
...
F_DC_PEER, F_DC_CHANNEL, F_DC_STATE, F_DC_LABEL = 1, 2, 7, 8
F_DC_TEXT, F_DC_DROPPED = 11, 13
E_MESSAGE = 8
...
# 2) Golden vector B: { F_DC_LABEL utf8 "ab" }
rec_b = kvrecord([f_utf8(F_DC_LABEL, "ab")])
GOLD_B = bytes([0x00, 0x01, 0x08, 0x02, 0x00, 0x02, 0x61, 0x62])
check("golden B bytes", rec_b == GOLD_B)

# 3) Golden vector A: { F_DC_DROPPED int 42, F_DC_STATE real 0.5 } (the real
#    is a pure FRAMING pin - no dcx field is real-typed today; the codec is).
rec_a = kvrecord([f_int(F_DC_DROPPED, 42), f_real(F_DC_STATE, 0.5)])
GOLD_A = bytes([0x00, 0x02, 0x0D, 0x00, 0x00, 0x02, 0x34, 0x32,
                0x07, 0x01, 0x00, 0x03, 0x30, 0x2E, 0x35])
check("golden A bytes", rec_a == GOLD_A)

# 4) Golden drain: 1 x E_MESSAGE { F_DC_PEER=7, F_DC_CHANNEL=3, F_DC_TEXT "hi" }
dr = drain([event_entry(E_MESSAGE,
                        [f_int(F_DC_PEER, 7), f_int(F_DC_CHANNEL, 3),
                         f_utf8(F_DC_TEXT, "hi")])])
GOLD_DRAIN = bytes([0x00, 0x01,
                    0x00, 0x08, 0x00, 0x12,
                    0x00, 0x03,
                    0x01, 0x00, 0x00, 0x01, 0x37,
                    0x02, 0x00, 0x00, 0x01, 0x33,
                    0x0B, 0x02, 0x00, 0x02, 0x68, 0x69])
...
check("roundtrip A dropped", fields[0] == (F_DC_DROPPED, FT_INT, b"42"))
check("roundtrip A state-real", fields[1] == (F_DC_STATE, FT_REAL, b"0.5"))

# 6) Decode the drain
ecount, off = read_u16(dr, 0)
check("drain count", ecount == 1)
etype, off = read_u16(dr, off)
check("drain type", etype == E_MESSAGE)
...
check("drain peer", fields[0] == (F_DC_PEER, FT_INT, b"7"))
check("drain channel", fields[1] == (F_DC_CHANNEL, FT_INT, b"3"))
check("drain text", fields[2] == (F_DC_TEXT, FT_UTF8, b"hi"))

# 7) Raw bytes survive verbatim (a binary message payload)
raw = kvrecord([f_raw(10, b"\xff\x80\x00")])
fields, off = read_kvrecord(raw, 0)
check("raw roundtrip", fields[0] == (10, FT_RAW, b"\xff\x80\x00"))

# 8) Empty value field is legal
empty = kvrecord([f_utf8(12, "")])
fields, off = read_kvrecord(empty, 0)
check("empty field", fields[0] == (12, FT_UTF8, b"") and off == len(empty))

if fails:
    print("%d golden failure(s)" % fails)
    sys.exit(1)
print("record_golden_test: all DataChannelXT wire-format vectors pinned OK")
sys.exit(0)
