/* dcx_handle_table.h — generation-tagged integer handle table, carried verbatim
 ...
 *
 * This matters MORE here than in any sibling binding: libdatachannel fires
 * callbacks from its own threads, and a callback can fire AFTER the script
 * layer freed the peer/channel it names. Events therefore carry handle IDS
 * (ints), never pointers, and every id is re-validated against this table when
 * the drain runs on the caller's thread (plan Part IV.5 "copy ids not
 * pointers"). NOTE the table itself is NOT thread-safe — the shim guards every
 * access (caller thread AND callback thread) with its single state mutex.
 *
 ...
 *
 * Header-only and libdatachannel-free, so it is unit-tested standalone under
 * ASan/UBSan (tests/record_handle_test.cpp). The shim instantiates one table
 * for peer connections and one for data channels.
 */
#ifndef DCX_HANDLE_TABLE_H
#define DCX_HANDLE_TABLE_H
...
public:
     ...
     * teardown sweep (dcx_cleanup frees every live peer/channel). */
