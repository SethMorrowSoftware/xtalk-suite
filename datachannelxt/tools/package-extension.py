...
WHAT IT DOES
    Copies a freshly built `datachannelxt.{so,dll,dylib}` into the committed
    extension tree at:

        src/code/<arch>-<platform>/datachannelxt.{so,dll,dylib}

    Installing the packaged extension makes the OXT engine resolve the
    `c:datachannelxt>` binding automatically via `the revLibraryMapping` - no loose
...
THE BARE-TOKEN NAME RULE
    The file is named with the bare token `datachannelxt` (NO `lib` prefix) so it
    matches the binding string. A build that emitted `libdatachannelxt.so` is wrong;
...
USAGE
    ...
    python3 tools/package-extension.py --platform-id universal-mac --lib out/datachannelxt.dylib
...
# (The committed file is always the bare token `datachannelxt` + this suffix.)
PLATFORM_SUFFIX = {
    "x86_64-linux": ".so",
    "x86-linux": ".so",
    "x86_64-win32": ".dll",
...
def _candidate_basenames(suffix):
    return ["datachannelxt" + suffix, "libdatachannelxt" + suffix]
...
def copy_into_tree(src_lib, platform_id, dry_run):
    ...
    dest = os.path.join(dest_dir, "datachannelxt" + suffix)  # ALWAYS the bare token
...
def assemble_staging(dry_run):
    ...
    stage(os.path.join("src", "datachannel.lcb"), "datachannel.lcb")
    ...
    if os.path.isdir(CODE_ROOT):
        for pid in VALID_PLATFORM_IDS:
            ...
            lib = os.path.join(CODE_ROOT, pid, "datachannelxt" + suffix)
            if os.path.exists(lib):
                actions.append((lib, os.path.join(staging, "code", pid, "datachannelxt" + suffix)))
...
def main(argv):
    ...
    suffix = PLATFORM_SUFFIX[platform_id]
    print("DataChannelXT package-extension: target platform-id = %s (datachannelxt%s)"
          % (platform_id, suffix))
    ...
    if args.lib:
        ...
        if src_lib is None:
            print("ERROR: no datachannelxt%s (or libdatachannelxt%s) found under %s.\n"
    ...
    if os.path.basename(src_lib).startswith("lib"):
        print("  note: source is lib-prefixed; writing the BARE-TOKEN name (datachannelxt%s)." % suffix)
