...
The OpenXTalk library suite: nine sibling extensions for OXT / the xTalk family,
...
openxtalk-libraries/
  ...
  start-here.livecodescript
                       ...
                       the front door prints its own record now. Later the
                       same day the LAUNCH became the sweep (opening a
                       SESSION stack closes the open holder first, keeping
                       the target itself), every newly opened demo lands at
                       the screen's center, and the card + log say which
                       suite extensions are actually LOADED (presence read
                       from the engine's own loadedExtensions list in a
                       try - the launcher still calls no native code, so a
                       row's "needs sodiumxt" can never read as missing on
                       a machine that has it); held
  ...
  riptide/             the capstone APP (not an extension): Riptide Social,
                       ...
                       a SIXTH rail 2026-08-29 (spec 8A) - the card broke
                       openStack on an engine, was reverted the same day, and
                       was RE-LANDED that same day with openStack
                       byte-identical to the engine-proven body, behind
                       riptide's tools/check-demo-boot.py, which BOOTS the
                       shipped stack headlessly (world model + capability
                       profiles) and mutation-proves its own fixtures: a
                       Nostr bridge over
