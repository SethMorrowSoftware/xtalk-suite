...
- **32-bit engines.** Arrange a 32-bit Windows OXT and a 32-bit Linux OXT for the `x86-win32` and
  ...
  may never have executed anywhere (CI's Windows KAT step is x86_64 only, and the Windows engine
  runs of 2026-09-24 and 2026-09-25 were 64-bit by the maintainer's account, so they loaded the
  `x86_64-win32` DLLs).
