...
| D-14 | `oxtkit/` shared native scaffolding: extract it, or retire the plan? | **Retire.** The one property it would buy, the three C++ handle tables never drifting, is held by `tools/check-shim-scaffold-drift.py`. Amended 2026-08-17: the tables normalise to 89 code lines and one digest, while the record codecs genuinely diverge (275/198/202 lines). Revisit only if an eighth native wrap is planned. **Not written through in code:** that tool's docstring and a `tools/build-all.sh` comment still call D-14 open. | 2026-08-27 | `docs/BINDING-PLAYBOOK.md` section 6 |
...
| D-18 | box2dxt's games and selftest, and holde-em's table: convert them to the suite kit chrome and scaffold, or make the exemptions permanent? | **Permanent.** The games are drawn by box2dxt's Kit, and the selftest matches the kit by value; converting would add a second 300-line block to every paste for no visual change. **Not written through in code:** the exemption reasons in `tools/check-ui-kit-drift.py` still say "phase-2". | 2026-08-27 | `box2dxt/CLAUDE.md` section 10; `holde-em/CLAUDE.md` rule 9 |
