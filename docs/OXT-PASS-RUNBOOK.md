...
recorded). A Linux engine ran the same paste green on 2026-09-25 with all six
present (section 8), but that record names neither the bitness nor whether the
committed `.so` files were the ones installed, so no Linux row has a record of
its own yet: record which DLL/`.so` row loaded, the bitness and where the
package came from; a regression is a finding about those builds, not about the
script.
...
| 47 | platform rows with no engine record | S5 (32-bit engines where they exist) | `x86-win32` for torrentxt and coinxt (coinxt's 32-bit DLL may never have executed, even in CI: the 2026-09-24 Windows run did not record its bitness) and sodiumxt (row 23); `x86-linux` for sodiumxt and torrentxt (the 2026-09-25 Linux run may have been either Linux row: its bitness was not recorded, so it closes neither). Green: the preflight LOADED and the member's sections green on that row, bitness recorded | the platform tables in each member's README and `CLAUDE.md` |
...
(section 8). A Linux engine loaded a Linux build of all six on 2026-09-25
(section 8), but which builds is not recorded: neither the bitness nor whether
the committed `.so` files or local builds were installed. torrentxt's was
built from 2026-09-08 source or later (INFERRED: it enforced the 996-byte BEP44
cap, which entered the shim that day and which, among committed Linux builds,
only the 2026-09-12 set carries). No engine has loaded the mac builds (row
24). onionxt, nostrxt, riptide and the apps are pure script.
...
- **Linux glibc floors** (the highest `GLIBC_` version each committed `.so`
  ...
  datachannelxt, nor torrentxt's 32-bit build. The 2026-09-25 Linux machine
  loaded all six and did not record its glibc: 2.38 or newer IF it ran the
  committed datachannelxt build (INFERRED), which the record does not say.
...
996-byte caps and both `btAddTorrentFile` forms included), on a library built
from 2026-09-08 source or later (INFERRED from the caps; the committed
2026-09-12 build if the committed package was installed, which the record does
not say) whose bitness and libtorrent version were not recorded (the tree pins
2.0.11 for Linux). A Linux run that names its row and build is still owed.
