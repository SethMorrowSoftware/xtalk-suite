...
| 16a | `coinxt/examples/coin-wallet.livecodescript` - the WALLET, ten screens. Its calculator layer is executed headlessly by `coinxt/tools/check-wallet-vectors.py` (twice: as written, and with `is`/`offset()` folded to the engine's case-insensitive default) and the STACK is booted headlessly by `coinxt/tools/check-wallet-boot.py`, so what this row is for is the part neither can reach: that the window builds and the show/hide sweep really shows one screen, that the QR image renders as a scannable symbol, and that a spend built here is accepted when broadcast. Nothing here has been broadcast to any network | coinxt packaged (the script layer, the wallet engine and onionxt are all embedded); sodiumxt for Generate and for the encrypted wallet file; a tor daemon only for the two Tor transports. **Use testnet** - script memory is not locked |
