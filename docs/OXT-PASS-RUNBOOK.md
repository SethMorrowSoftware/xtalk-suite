...
2026-09-24 and again on 2026-09-25 (section 8; a 64-bit OXT, by the
maintainer's account). The `x86_64-linux` builds of all six (sodiumxt's and
box2dxt's still release run 12's) first met an engine on the record on
2026-09-25: 64-bit Kubuntu 24.04 with "the latest builds", by the maintainer's
account (section 8). torrentxt's enforced the 996-byte BEP44 cap, which entered
the shim on 2026-09-08 and which, among committed Linux builds, only the
...
- **Windows.** The DLLs carry different upstream versions from Linux and mac:
  ...
  Both loaded on 2026-09-24 and again on 2026-09-25 by the maintainer's account
  (section 8: sodiumxt 106/106, torrentxt 106/106; the report prints no library
  version). Those runs were 64-bit, by the maintainer's account given
  2026-09-26, so they are the `x86_64-win32` rows' records; the `x86-win32`
  DLLs wait on a 32-bit OXT (rows 23 and 47).
...
DLL (a 64-bit OXT, by the maintainer's account given 2026-09-26); the same DLL
read 106/106 again on 2026-09-25. The Windows DLLs have carried 2.1.1 since
2026-08-27, and that day's paste recorded neither its platform nor its
binaries. Linux followed on 2026-09-25 (the suite paste, 106/106, the 996-byte
caps and both `btAddTorrentFile` forms included), on the 2026-09-12
...
other sections read as Linux's (section 8): four stalls in four runs there,
none on Linux. Which, only that machine can say: the independent test is each
transport's own loopback there, the standalone
