...
2026-09-24, on Windows (the suite paste, section 8; the bitness was not
recorded), and their `x86_64-linux` builds (sodiumxt's and box2dxt's still
release run 12's) on 2026-09-25, the same paste green on 64-bit Kubuntu 24.04
with "the latest builds" by the maintainer's account (section 8). Still no
record: `x86-linux` (a 32-bit OXT), either Windows DLL by bitness, and any Mac.
Record which DLL/`.so` row loaded, the bitness and where the package came from;
a regression is a finding about those builds, not about the script.

| # | Run | Expect / record | ~min |
|---|---|---|---|
| 0 | `tests/preflight.livecodescript` (3.2) | six LOADED; Box2Dxt found 4 | 5 |
| 1 | `tests/suite-selftest.livecodescript`, then its board (3.1) | RECORD every member total (below; the summary prints them per row), do not match it; wait for `summary` (4.1.1). Then row 48's open half in the same launch: a second Run all, three rows' Run, each filter | 45 |
...
Item 1's last records (2026-09-25, Linux; section 8), to record against rather
...
are the live anon-service legs), box2dxt 385/0 at harness v32, holde-em 751/0/5
at v0.25.5 / harness 47 (the 5 skips are its live-transport legs). Whole paste:
2672/0/10 (2682), both live loopbacks completing. The three Windows runs of
2026-09-24 read 2620/5/3, 2623/2/3 (riptide's two fixes in) and 2623/2/10 (the
skips merged), with holde-em 721/0 at v0.25.3 / harness 45, and all three
runs' loopbacks stalled (5.5: the same loopback code completed on Linux, so
the paste's code alone does not explain the stall). A new total is not a
regression by itself; a red line is.
...
| 23 | sodiumxt's Windows DLLs: MSVC builds with libsodium 1.0.22, committed 2026-09-12 (the 2026-08-24 x64 proof ran on a mingw DLL that no longer ships); PARTLY RUN 2026-09-24 (section 8): 106/106 on ONE of the two, by the maintainer's account, with neither the bitness nor `sxVersion()` recorded, so neither row closes | S5 | On `x86_64-win32` AND `x86-win32`, each named by its bitness: LOADED at ABI 10; the full SodiumXT section (106) green, including SHA3, ristretto and the ABI-10 ChaCha20 xor; `put sxVersion()` shows `libsodium 1.0.22`. A 64-bit-only day proves only `x86_64-win32` | the Windows rows of `sodiumxt/CLAUDE.md`'s committed-binaries table; the root README's 32-bit note |
...
2026-09-12 release run, whose Windows builds first met an engine on 2026-09-24
(section 8). The `x86_64-linux` builds of all six (sodiumxt's and box2dxt's
still release run 12's) first met an engine on the record on 2026-09-25: 64-bit
Kubuntu 24.04 with "the latest builds", by the maintainer's account (section 8).
torrentxt's enforced the 996-byte BEP44 cap, which entered the shim on
2026-09-08 and which, among committed Linux builds, only the 2026-09-12 set
carries, so that account and the report agree. No engine has loaded the
`x86-linux` builds (row 47) or the mac builds (row 24). onionxt, nostrxt,
...
nor its binaries. Linux followed on 2026-09-25 (the suite paste, 106/106, the
...
Linux library (row 47) is still owed.
...
code alone. Which, only that machine can say: the independent test is each
