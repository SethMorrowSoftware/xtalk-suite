...
numbers). The FOLDED run is still first-time; the remaining live legs are the
probe's own (five legs since the rewrite: click Live probe, copy the log) plus
a Search in the Films family for the async `search` kind, and unlike nostrxt
this member's other half needs a NETWORK, so the pass splits cleanly. The folded run rides the
