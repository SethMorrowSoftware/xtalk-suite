...
| Q | coinxt ABI 7 and silent-payment RECEIVING (added 2026-09-10): reinstall the five rebuilt binaries (the mac dylib is the first ever cross-built off a Mac - Zig compile, ld64.lld link), paste `coinxt/tests/coin-selftest.livecodescript` (or the suite paste) and read the "secp256k1 keys" section, then open `coinxt/examples/coin-wallet.livecodescript` with the test seed on testnet: Receive shows a `tsp1...` address under the derivation and "Copy silent-payment address" fills the clipboard; on Tools paste a transaction on line 1 and one prevout script per input below it and press Inspect | the six `cxPubkeyCombine` lines green (G+G is 2G, one key is itself, the intermediate-infinity sum, and the three refusals) - which is the first `Data`-of-many-keys argument shape this binding has marshalled; the wallet prints the address the boot self-check derived; Inspect on a real transaction paying it reports `FOUND:` with the taproot address, the Addresses screen gains the record, Save keeps it as an `sp` line | promote `cxPubkeyCombine` and `cwSpScan` from "verified statically" in `coinxt/CLAUDE.md` (the 2026-09-10 entries) and `coinxt/docs/wallet.md`; a refusal on the mac dylib is a finding about the cross-build, not about the code - say which. Tick: ABI7 | 10 |
