...
| Q | coinxt ABI 7 and silent-payment RECEIVING (added 2026-09-10): reinstall the five rebuilt binaries (the mac dylib is the first ever cross-built off a Mac - Zig compile, ld64.lld link), paste `coinxt/tests/coin-selftest.livecodescript` (or the suite paste) and read the "secp256k1 keys" section, then open `coinxt/examples/coin-wallet.livecodescript` with the test seed on testnet: Receive shows a `tsp1...` address under the derivation and "Copy silent-payment address" fills the clipboard; on Tools, with a backend chosen, paste the paying transaction ALONE and press Inspect (since 2026-09-11 the wallet asks the backend for each parent and runs the scan when the last one lands, repainting the result box from the socket callback; offline, paste one prevout script per input below the transaction instead - the 2026-09-10 shape) | the six `cxPubkeyCombine` lines green (G+G is 2G, one key is itself, the intermediate-infinity sum, and the three refusals) - which is the first `Data`-of-many-keys argument shape this binding has marshalled; the wallet prints the address the boot self-check derived; Inspect on a real transaction paying it first prints "Asked <host> for the N transaction(s) these inputs spend", then the log says "finished" and the Tools result box repaints with `FOUND:` and the taproot address (whatever screen was showing when the answer came - the repaint from a socket callback is the unproven half), the Addresses screen gains the record, Save keeps it as an `sp` line; a second Inspect scans at once without a request | promote `cxPubkeyCombine` and `cwSpScan` from "verified statically" in `coinxt/CLAUDE.md` (the 2026-09-10 entries) and `coinxt/docs/wallet.md`; a refusal on the mac dylib is a finding about the cross-build, not about the code - say which. Tick: ABI7 | 10 |
...
added later, and the table now runs 11-35).** They were ALL OPEN when this
...
| 16a | `coinxt/examples/coin-wallet.livecodescript` - the WALLET, ten screens. Its calculator layer is executed headlessly by `coinxt/tools/check-wallet-vectors.py` (twice: as written, and with `is`/`offset()` folded to the engine's case-insensitive default) and the STACK is booted headlessly by `coinxt/tools/check-wallet-boot.py`, so what this row is for is the part neither can reach: that the window builds and the show/hide sweep really shows one screen, that the QR image renders as a scannable symbol, and that a spend built here is accepted when broadcast. Nothing here had been broadcast to any network until 2026-09-02, when a pasted engine log showed a testnet spend (txid `7978bdd2c097c929cae2ab00084d4454b68b1d054a3f2d53fc7b51b70551e4d5`) accepted by the Esplora onion mirror over Tor and seen spent by the sync after it; Electrum-over-Tor is the one transport still unproven | coinxt packaged (the script layer, the wallet engine and onionxt are all embedded); sodiumxt for Generate and for the encrypted wallet file; a tor daemon only for the two Tor transports. **Use testnet** - script memory is not locked |
...
### 4.10 archivext (inventory item 36 - first engine contact 2026-09-15, standalone; the fold and the live legs still owed)

The standalone harness has run once (357/2/0, both reds fixed the same day -
engine notes 2.7 and `archivext/CLAUDE.md` gotchas 15-16); the FOLDED run and
the live legs are still first-time, and unlike nostrxt this member's other
half needs a NETWORK, so the pass splits cleanly. The folded run rides the
...
engine record). The TLS result - WHATEVER it is - goes to
`docs/OXT-ENGINE-NOTES.md` section 6 beside 6.8, as the suite's first libURL
https measurement.
...
[ ] ABI7 (S1 row Q; coinxt ABI 7 cxPubkeyCombine and the silent-payment receiver, both verified statically until this line is filled):
