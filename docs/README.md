...
| move a member into its own repository, or delete one from this tree | [MEMBER-REPO-SPLIT.md](MEMBER-REPO-SPLIT.md) |
...
| [MEMBER-REPO-SPLIT.md](MEMBER-REPO-SPLIT.md) | LIVE | whole suite | The split of every member into its own repository: what "ready" means and the gate that holds it (`tools/check-member-standalone.py`), the sibling layout the cross-member gates expect (`../<name>`, `XTALK_SIBLINGS`), the target repositories (from `tools/member-registry.py`), the export procedure, the registries a departing member must be removed from (learned from archivext, 2026-09-21), and what a standalone repository cannot check on its own. |
