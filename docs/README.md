...
| ask whether the engine can run without a human at a GUI | [HEADLESS-ENGINE.md](HEADLESS-ENGINE.md) |
...
| [HEADLESS-ENGINE.md](HEADLESS-ENGINE.md) | LIVE | whole suite | Whether the engine can be driven with no human at a GUI, and what changes if it can. Prompted by a 2026-08-27 forum reply from Brian Milby saying the server engine and the LCB compiler both run headless and that editors already lint LiveCodeScript that way. Carries the honest sizing (6 of the engine notes' 26 entries are compile-time and 2 more only with `the explicitVariables`; the other 18 are runtime semantics no compiler sees), the capability probe that turns the guesses into dated evidence, the staged plan with abort criteria, and what stays manual regardless. **No engine has run any of it** - every capability claim in it is DOCUMENTED or weaker, which is why the tree's no-headless assertions are left untouched (`tools/check-premise-count.py` counts them, with its convention). |
