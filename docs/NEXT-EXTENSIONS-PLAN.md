...
|---|---|---|---|
| Discovery + bulk transfer | **TorrentXT** (libtorrent) | DHT rendezvous + BitTorrent file movement | done (ABI v11: the v9-v11 surface added signed puts, rendezvous, NAT mapping, rp1) |
| Trust | **libsodium** | identity, signing, encryption, password hashing | done (SodiumXT / org.openxtalk.library.sodium) |
...
| Real-time (WebRTC) | **libdatachannel** + **libjuice** | browser-interoperable P2P + real NAT traversal | **Phases 1-2 built** - `datachannelxt/` (standalone-ready folder; dcx_ ABI v1, data channels only; smoke test green under ASan/UBSan + TSan). Engine evidence is layered, not one bit: the binding + a live in-process loopback ran green 2026-08-08, and the folded synchronous half (all 31 handlers by name) 2026-08-10; the member harness's standalone async loopback closed 2026-08-15 (nothing in the selftest is static anymore; open items are browser interop and a two-network call). The IV.7 flagship demo - P2P chat over **DHT-signalled** WebRTC, TorrentXT BEP44 as the rendezvous, `datachannelxt/examples/datachannel-dht-chat.livecodescript` - is itself still verified statically (its UI has not run), and its real two-machine, cross-NAT DHT signaling is runbook item 6 |
