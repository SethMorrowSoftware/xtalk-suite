...
| 14 | Code-comment truth-sync: the `torrent-selftest` header says "runs 96 checks" (a green run counts 101 since 2026-08-17: 45 in `stRun` + 56 in `stRunTorrentChecks`; re-count after coding #3 lands). The three demo headers and `tests/onion_frame_golden.py` were fixed 2026-09-24 | Stale claims in shipped files | S | none |
