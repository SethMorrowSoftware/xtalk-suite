...
The engine cannot be driven headlessly, so every change to the embedded web host is flagged
*"verified statically; needs an OXT pass."* This is that pass. The member's three gates (the
static checker `tools/check-livecodescript.py`, the golden `tests/fileserver_golden.py`, and the
execution gate `tools/check-script-vectors.py`, which runs the pure helpers through the family's
interpreter) settle logic; they **cannot** observe the runtime behaviours below, which need a real
OpenXTalk engine.

It covers the HTTP host (custom `.qsroutes.json` routes including `:param` patterns, the `/_qs/*`
endpoints, the header set, `OPTIONS`/`Allow`/`405`, `HEAD`, the reserved namespaces, conditional
GET, CORS preflight, the editor and its throttle), the invariants those ride on (self-building
UI, fail-closed extensions, clean shutdown), and the webapp's browser-only claims in section 7.
...
- [ ] **`HEAD` on a ROUTE** (fixed 2026-08-17; verified statically + golden-pinned, THIS is its
      ...
      and never the SPA's `index.html` (the pre-fix symptom; `qsRouteLookupKey` now maps HEAD
      onto the GET route). Check over **both** transports.
- [ ] **`HEAD` over Tor specifically** (`qsFsSendText`, fixed the same day): `HEAD /` on a folder
      ...
      (compare `curl -s ... | wc -c` for the GET against the `HEAD`). A body here would not
      desync anything (a Tor response closes its stream), so *measure the byte count*; do not
      just look for a working page.
...
- [ ] **Admin link** in the footer routes to the admin panel (see section 6a).
...
- [ ] Without **SodiumXT** -> encryption / editor / Tor off with a clear message; Share-code and
      Web-link still work.
- [ ] **No Tor daemon running** (neither system tor nor Tor Browser) -> the Tor pill reads
      "Tor: not running", the Private - Tor choice is disabled; Share code and Web link work.
- [ ] Without **TorrentXT** (guarded 2026-09-09, verified statically - THIS is its engine pass)
      ...
      and there is NO raw engine dialog (the pre-guard symptom).
