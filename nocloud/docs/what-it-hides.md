...
9151) **and SodiumXT** - without SodiumXT the whole Tor path is off, not just its
passphrase features. The OnionXT layer itself is nothing you have to fetch - it is
carried inside the app's own script. When those pieces are missing the path simply
isn't offered, and the other two paths keep working.
